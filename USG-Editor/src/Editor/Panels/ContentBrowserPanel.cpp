...
namespace Editor
{
	...
	void ContentBrowserPanel::OnImGuiRender()
	{
		...
		for (auto& p : fs::directory_iterator(m_CurrentDirectory))
		{
			...
			if (ImGui::BeginPopupContextItem(("##" + std::to_string(itemID)).c_str()))
			{
				if (ImGui::MenuItem("Remove"))
					Engine::Application::Get().GetAssetManager().DeleteAsset(path);
				ImGui::EndPopup();
			}
			...
			if (ImGui::IsItemHovered())
			{
				if (ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left))
				{
					if (p.is_directory())
						...
						Engine::Application::Get().GetAssetManager().OpenAsset(path);
			...
			if (!m_OldFileName.empty() && m_OldFileName == filename)
			{
				...
				if (ImGui::InputText("##ChangeFileName", &m_NewFileName, ImGuiInputTextFlags_AutoSelectAll | ImGuiInputTextFlags_EnterReturnsTrue)
					|| Engine::Input::GetKeyPressed(Engine::KeyCode::ENTER) || (ImGui::IsMouseClicked(ImGuiMouseButton_Left) && !ImGui::IsItemHovered()))
				{
					if (m_NewFileName != m_OldFileName)
						Engine::Application::Get().GetAssetManager().RenameAsset(path, m_NewFileName);
					m_OldFileName.clear();
				}
		...
		if (ImGui::BeginPopupContextWindow(0, 1, false))
		{
			if (ImGui::BeginMenu("Add"))
			{
				...
				{
					m_OldFileName = Engine::Application::Get().GetAssetManager().CreateFolder(m_CurrentDirectory / "New Folder").string();
	...
	Engine::Ref<Engine::Texture2D> ContentBrowserPanel::GetFileIcon(const fs::directory_entry& file)
	{
		...
		if (Engine::Texture2D::ValidExtension(ext))
			return Engine::Application::Get().GetAssetManager().GetAsset<Engine::Texture2D>(file.path());
		...
		if (ImGui::BeginDragDropTarget())
		{
			...
			if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("CONTENT_BROWSER_ITEM"))
			{
				...
				if (droppath.string().rfind(path.string(), 0) != 0)
					Engine::Application::Get().GetAssetManager().MoveAsset(droppath, path);
			}
