...
namespace Game
{
	...
	void StaticModelRendererComponent::OnComponentRemoved()
	{
		for (uint32 i = 0; i < m_CollapsedModelTree.Count(); i++)
		{
			...
			Owner.GetScene()->GetSceneRendererAs<SceneRenderer>()->GetMainPass().RemoveObject(entry.m_Object);
		}
	}
	...
	void StaticModelRendererComponent::SetModel(Engine::Ref<Model> model)
	{
		...
		// remove old model
		for (uint32 i = 0; i < m_CollapsedModelTree.Count(); i++)
			Owner.GetScene()->GetSceneRendererAs<SceneRenderer>()->GetMainPass().RemoveObject(m_CollapsedModelTree[i].m_Object);
